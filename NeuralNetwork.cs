...
public class Network {
    ...
    public void Mutate(double percentage) {
        ...
        for (int i = 0; i < input_layers; i++) {
            for (int h = 0; h < hidden_layers; h++) {
                input_hidden_weights[i, h] += Random_neg_to_pos() * (percentage * input_hidden_weights[i, h]);
            }
        }

        for (int h = 0; h < hidden_layers; h++) {
            for (int o = 0; o < output_layers; o++) {
                hidden_output_weights[h, o] += Random_neg_to_pos() * (percentage * hidden_output_weights[h, o]);
            }
        }
